...
def get_past_balance():
    ...
    for account in asset_report_json['items'][0]['accounts']:
        account_name = account['name']
        positive_percentage = calculate_positive_percentage(account)
        account_stats.append({'account': account_name, 'percentPositive': str(round(positive_percentage * 100, 2)) + '%'})

    print(account_stats)
    return jsonify({'accounts': account_stats})



def calculate_positive_percentage(account):
    ...
    if total == 0:
        return 1
    return float(positive_balance)/total
